...
async function init() {
  ...
  // Global referans — exam type menüsünden erişim için
  window._handleSendMessage = handleSendMessage;

  // Cihaz modu yükleme
  const savedMode = localStorage.getItem('ata_device_mode');
  if (savedMode && typeof applyDeviceMode === 'function') {
        ...
        const dHtml = `
          <div class="welcome-dash">
            ...
            <div class="dash-grid">
               ...
               <button class="dash-card-btn c3" onclick="window._openExamTypeMenu()">
                 <div class="dash-card-icon">📝</div>
                 <div class="dash-card-content">
                    <div class="dash-card-title">3. BÖLÜM (Sınav Yap)</div>
                    <div class="dash-card-desc">Soru tipi seç, sınava hazırlan</div>
                 </div>
               </button>
               <button class="dash-card-btn c4" onclick="document.getElementById('btnOpenVoiceExam')?.click()">
                 <div class="dash-card-icon">🎤</div>
