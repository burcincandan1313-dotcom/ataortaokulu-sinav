  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      ...
      lines.forEach((line, i) => {
        ...
        if (hasMojibake) {
           results.push({ file: fullPath, lineNum: i + 1, content: line.trim(), match });
        }
      });
    }
  }
...
if (fs.existsSync('index.html')) {
   ...
   text.split('\n').forEach((line, i) => {
      ...
      if(hasMojibake) results.push({ file: 'index.html', lineNum: i + 1, content: line.trim(), match });
   });
}
